...
Currently, we use round robin strategy to put all kinds of hadoop cluster's VMs into vSphere Cloud.
We have five type resources in vSphere: clusters, resource pools, hosts, datastores and networking.
We use loop_resource() to round robin by those resources. Firstly, we round robin resouce pools in
one cluster. In the resource pool, we round robin every host. In one host, we round robin every
